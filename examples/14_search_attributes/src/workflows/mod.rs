...
use crate::activities::{InventoryResult, OrderInfo, OrderProcessingResult, OrderStatus};
...
pub async fn order_processing_with_search_attrs_workflow(
    ...
    if !inventory.all_available {
        ...
        let cancel_result = ctx
            .execute_activity(
                "cancel_order",
                Some(
                    serde_json::to_vec(&(order.order_id.clone(), "Items unavailable".to_string()))
                        .unwrap(),
                ),
                ActivityOptions::default(),
            )
    ...
    info!(
        "Order {} validated, updating search attributes",
        order.order_id
    );
    ...
    if !matches!(order.priority, crate::activities::OrderPriority::Critical) {
        ...
        info!(
            "Order {} shipped, updating search attributes to shipped",
            order.order_id
        );
    ...
    info!(
        "Order {} workflow completed with status: {:?}",
        order.order_id, processed.status
    );
...
pub async fn priority_order_workflow(
    ...
    info!(
        "Setting priority search attribute to level {}",
        priority_level
    );
