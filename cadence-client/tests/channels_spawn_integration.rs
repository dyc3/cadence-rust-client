            ...
            for job in input_data.jobs {
                ...
                ctx.spawn(async move {
                    ...
                    let job_bytes = serde_json::to_vec(&job)
                        .map_err(|e| cadence_workflow::WorkflowError::Generic(e.to_string()))?;
                    ...
                    Ok::<_, cadence_workflow::WorkflowError>(())
                });
            }
                        ...
                        Ok(result_bytes) => {
                            let result: u32 =
                                serde_json::from_slice(&result_bytes).map_err(|e| {
                                    cadence_workflow::WorkflowError::Generic(e.to_string())
                                })?;
                            Ok(result)
                        }
                        Err(e) => Err(e),
...
async fn test_split_merge_workflow_with_channels_and_spawn() {
    ...
    tracing_subscriber::fmt()
        .with_env_filter("info,cadence_worker=debug,cadence_workflow=debug")
        .with_test_writer()
        .try_init()
        .ok();
