    ...
    pub async fn handle(
        ...
        let execution_result = match result {
            ...
            Err(panic_info) => {
                ...
                Err(CadenceError::Other(format!("Workflow panic: {}", panic_msg)))
            }
        };
        ...
        match execution_result {
            Ok((decisions, query_results)) => {
                ...
                println!("[DecisionTaskHandler] Responding with {} decisions", decisions.len());
                let response = self.service.respond_decision_task_completed(RespondDecisionTaskCompletedRequest {
                ...
                match &response {
                    Ok(_) => println!("[DecisionTaskHandler] Successfully responded to decision task"),
                    Err(e) => println!("[DecisionTaskHandler] FAILED to respond to decision task: {:?}", e),
                }
            ...
            Err(e) => {
                ...
                let _ = self.service.respond_decision_task_failed(RespondDecisionTaskFailedRequest {
                    task_token: task.task_token,
                    cause: cadence_proto::shared::DecisionTaskFailedCause::WorkflowWorkerUnhandledFailure,
