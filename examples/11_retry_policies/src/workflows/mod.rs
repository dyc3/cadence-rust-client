...
use crate::activities::{PaymentRequest, PaymentResult, RetryableOperationResult};
...
pub async fn validation_before_processing_workflow(
    ...
    match validation_result {
        Ok(result) => {
            let validation: RetryableOperationResult =
...
pub async fn timeout_handling_workflow(
    ...
    info!(
        "Starting timeout handling workflow (expected duration: {}ms)",
        duration_ms
    );
