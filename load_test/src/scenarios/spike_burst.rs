...
use cadence_proto::shared::{TaskList, TaskListKind, WorkflowType};
...
pub async fn run(client_args: ClientArgs, args: SpikeArgs) -> Result<()> {
    ...
    tracing::info!("Starting spike pattern: baseline={}/sec, spike={}/sec", 
                   args.baseline_rate, args.spike_rate);
    ...
    loop {
        ...
        if elapsed_in_interval >= interval_duration {
            ...
            let rate = if in_spike { args.spike_rate } else { args.baseline_rate };
            tracing::info!("Switching to {} mode: {}/sec", 
                          if in_spike { "SPIKE" } else { "baseline" }, rate);
        }
        
        // Get current rate
        let current_rate = if in_spike { args.spike_rate } else { args.baseline_rate };
