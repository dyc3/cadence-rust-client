...
pub fn workflow(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        impl cadence_worker::registry::Workflow for #wrapper_ident {
            fn execute(
                &self,
                ctx: cadence_workflow::WorkflowContext,
                input: Option<Vec<u8>>,
            ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Vec<u8>, cadence_worker::registry::WorkflowError>> + Send>> {
                Box::pin(async move {
                    let input_data = input.ok_or_else(|| cadence_worker::registry::WorkflowError::ExecutionFailed("Missing input".to_string()))?;
                    let decoded = serde_json::from_slice(&input_data)
                        .map_err(|e| cadence_worker::registry::WorkflowError::ExecutionFailed(e.to_string()))?;
        ...
        #vis mod #module_ident {
            ...
            pub fn register(registry: &dyn cadence_worker::registry::Registry) {
...
pub fn activity(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        impl cadence_worker::registry::Activity for #wrapper_ident {
            fn execute(
                &self,
                ctx: &cadence_activity::ActivityContext,
                input: Option<Vec<u8>>,
            ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Vec<u8>, cadence_worker::registry::ActivityError>> + Send>> {
                let ctx = ctx.clone();
                Box::pin(async move {
                    let input_data = input.ok_or_else(|| cadence_worker::registry::ActivityError::ExecutionFailed("Missing input".to_string()))?;
                    let decoded = serde_json::from_slice(&input_data)
                        .map_err(|e| cadence_worker::registry::ActivityError::ExecutionFailed(e.to_string()))?;
                    let output = #ident(&ctx, decoded).await?;
                    serde_json::to_vec(&output)
                        .map_err(|e| cadence_worker::registry::ActivityError::ExecutionFailed(e.to_string()))
                })
    ...
    let expanded = quote! {
        async {
            let bytes = serde_json::to_vec(&#payload)
                .map_err(|e| cadence_workflow::WorkflowError::ExecutionFailed(e.to_string()))?;
