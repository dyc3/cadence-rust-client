...
pub async fn order_processing_saga(
    ...
    info!(
        "Starting order processing saga: {} for user {}",
    ...
    match payment_result {
        Ok(result) => {
            ...
            let notification: NotificationResult = serde_json::from_slice(&notification_result)
                .map_err(|e| {
                    WorkflowError::Generic(format!("Failed to parse notification: {}", e))
                })?;

            saga_state
                .notifications_sent
                .push(notification.notification_id);
...
pub async fn order_fulfillment_workflow(
    ...
    info!(
        "Order fulfillment complete: {} with tracking {}",
