...
async fn test_get_workflow_execution_history() {
    ...
    let has_workflow_started = history
        .events
        .iter()
        .any(|e| e.event_type == uber_cadence_proto::shared::EventType::WorkflowExecutionStarted);

    let has_decision_scheduled = history
        .events
        .iter()
        .any(|e| e.event_type == uber_cadence_proto::shared::EventType::DecisionTaskScheduled);
